...
@variable(scnd, 0<= y[1:dt.N["plant"]+dt.N["distribution"],1:dt.N["transportation"]] <= 1 );
...
@variable(scnd, 0<= h[1:dt.N["plant"]+dt.N["distribution"],1:5,1:dt.N["transportation"]] );
...
for i=1:dt.N["supplier"]
    ...
    for j=1:dt.N["plant"]
        ...
        # end
        for m=1:dt.Mij[i,j]
            add_to_expression!(exv,sum(dot.(dt.N["tcp"][i][idx:idx+4],xij[i,j,m,:])) );
            idx+=5
        end
    end
end
for j=1:dt.N["plant"]
    ...
    for k=1:dt.N["distribution"]
        ...
        # end
        for m=1:dt.Mjk[j,k]
            add_to_expression!(exv,sum(dot.(dt.N["tcd"][j][idx:idx+4],xjk[j,k,m,:])) );
            idx+=5
        end
    end
end
...
# for j=1:dt.N["plant"]
#     for p=1:5
#         @constraint(scnd, [j=1:dt.N["plant"],p=1:5], sum(xij[:,j,:,p]) == sum(xjk[j,:,:,p]) )
#     end
# end
########### constraint 4-6 #############
@constraints(scnd, begin
...
@constraint(scnd,[j=1:dt.N["plant"]+dt.N["distribution"], t=1:dt.N["transportation"]], sum(h[j,:,t]) <= [dt.N["cap"];dt.N["cad"]][j]*y[j,t])
...
########### constraint 13-14 #############
@constraint(scnd, sum(y[j,t] for j=1:dt.N["plant"] for t=1:dt.N["transportation"]) <= dt.upl)
@constraint(scnd, sum(y[j,t] for j=dt.N["plant"]+1:dt.N["distribution"]+dt.N["plant"] for t=1:dt.N["transportation"]) <= dt.udc)

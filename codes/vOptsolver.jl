using DataFrames,DelimitedFiles,JuMP,LinearAlgebra,CPLEX,MathOptInterface,vOptGeneric,SparseArrays
# using CPUTime
struct Data2
...
function build_1dim()
    ...
    @addobjective(scnd1, Min, sum(dt1.c.*y1) +sum(repeat(dt1.a[1,:], outer=sum(dt1.Mij[1,:])).*xij1[1:sum(dt1.Mij[1,:])*5])+
    ...
    @addobjective(scnd1, Min, sum(repeat(dt1.b[1,:], outer=sum(dt1.Mij[1,:])).*xij1[1:sum(dt1.Mij[1,:])*5]) +

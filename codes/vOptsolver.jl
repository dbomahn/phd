...
struct Data1dim
    ...
    function Data1dim(file)
        ...
        notafile = readdlm("D:/scnd/Notations.txt", '=');
...
function FPplus(dvar,len,TL,Y_N) #𝚯
    X = copy(dvar); PF = copy(Y_N); Y = []; IGPair=[]; Tabu = []; newsol=0; t0=time();
    # U1 = []; U2= []; U3 = [];
...
FPtime = @CPUelapsed fx,fy,fn,usedPairs = FPplus(vd.X_E,len,round(Int,Dichotime/8),vd.Y_N)
################################ Path Relinking  ###############################
...
PRtime = @CPUelapsed px,py,pn,pairs = PR(fx,fy,len,round(Int,FPtime*2))
...
CSV.write("/home/k2g00/k2g3475/scnd/vopt/mipY/"*name*"mipY.log", DataFrame(otable, :auto), append=false, header=false,delim=' ')
println("time $name: ", Dichotime+FPtime+PRtime, ": #sol: ", length(pry))
...
function voptmodel()
    ...
    ########### constraint 10 #############
    @constraint(scnd,[i=1:dt.N["supplier"],j=1:dt.N["plant"]], sum(uij[i,j,m] for m=1:dt.Mij[i,j]) <= sum(yj[j,:]));
    @constraint(scnd,[j=1:dt.N["plant"],k=1:dt.N["distribution"]], sum(ujk[j,k,m] for m=1:dt.Mjk[j,k]) <= (sum(yj[j,:])+sum(yk[k,:]))/2 );
    @constraint(scnd,[k=1:dt.N["distribution"],l=1:dt.N["customer"]], sum(ukl[k,l,m] for m=1:dt.Mkl[k,l]) <= sum(yk[k,:]));
    ########### constraint 11 #############
...
scndm = voptmodel()
# using vOptGeneric
@CPUtime vSolve(scndm, 500, method=:dicho, verbose=false)
# MIPtime = @CPUelapsed vSolve(scndm, 500, method=:lex, verbose=false)

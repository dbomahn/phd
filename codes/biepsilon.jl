...
#########################  IP  ########################################
@variable(test, y1[1:(dt1.N["plant"]+dt1.N["distribution"])*2], Bin)
@variable(test, uij1[1:sum(dt1.Mij)], Bin);
@variable(test, ujk1[1:sum(dt1.Mjk)], Bin);
@variable(test, ukl1[1:sum(dt1.Mkl)], Bin);
@variable( test, 0<= xij1[1:sum(dt1.Mij)*5] );
@variable( test, 0<= xjk1[1:sum(dt1.Mjk)*5] );
@variable( test, 0<= xkl1[1:sum(dt1.Mkl)*5] );
@variable( test, 0<= h1[1:(dt1.N["plant"]+dt1.N["distribution"])*5*2] );
obj2 = @expression(test,sum(repeat(dt1.b[1,:], outer=sum(dt1.Mij[1,:])).*xij1[1:sum(dt1.Mij[1,:])*5]) +
...
function getobjval(model,num)
    ...
    ukl1 = value.(model[:ukl1]);
    xij1 = value.(model[:xij1]);
...
function lexobj1(TL)
    ...
    obj2 = @expression(lex,sum(repeat(dt1.b[1,:], outer=sum(dt1.Mij[1,:])).*xij1[1:sum(dt1.Mij[1,:])*5]) +
            ...
            sum(dt1.q.*h1) + sum(dt1.rij.*xij1)+sum(dt1.rjk.*xjk1)+sum(dt1.rkl.*xkl1))
    # @variable(lex, 0 <= ep);
    # @constraint(lex, epcon1, obj2 <= ep);
    @objective(lex, Min, obj1)
    # @constraint(lex, epcon1, obj1 <= ep);
...
function epmodel1dim(TL)
    ...
    set_optimizer_attribute(scnd1, "CPXPARAM_TimeLimit", TL)
    #########################  IP  ########################################
...
function opt1dim(ϵ)
    JuMP.fix(m2[:ep], ϵ; force = true);
    optimize!(m2)
    # @show termination_status(m2)
    if has_values(m2) == true
        obj2 = getobjval(m2,2)
...
function epsilon(Y,lex1,lex2,step)
    ϵ = lex1-step; δ =step; lb = lex2; fval = [0,ϵ]
    while length(Y) < 11 && fval[2] > lb
        fval = opt1dim(ϵ)
        println(fval)
        if fval == 0
            break
...
CSV.write(name*"newY.log", DataFrame(otable, :auto), append=false, header=false, delim=' ')
...
###########
# @show file = ARGS[1]
file = "/home/k2g00/k2g3475/scnd/instances/test14S3"
dt = mdData(file);
TL = 3600*3
...
function lexobj1(TL)
    ...
    set_optimizer_attribute(scnd, "CPXPARAM_TimeLimit", TL)

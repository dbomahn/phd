...
function giveobjval(x,FLP::Data)
    if x!=nothing
        return [dot(x[1:FLP.i],FLP.fixcost[:]),-dot(x[FLP.i+1:FLP.j+FLP.i],FLP.demand[:]),dot(x[FLP.j+FLP.i+1:end],(FLP.cost[:]))]
    else
        return nothing
    end
end
...
function FLPlex(FLP::Data)::Problem
    ...
    @constraint(flp, con3, sum(FLP.fixcost[a]*y[a] for a=1:FLP.i) <= obj1 )
    @constraint(flp, con4 , -sum(z[b]*FLP.demand[b] for b=1:FLP.j) <= obj2 )
    @constraint(flp, epsilon11,  -sum(z[b]*FLP.demand[b] for b=1:FLP.j) <= ep′[1]-1)
    @constraint(flp, epsilon12,  ep[1] <= -sum(z[b]*FLP.demand[b] for b=1:FLP.j) )
...
mutable struct Laumanns
    ...
    function Laumanns()
        TimeLimit = 7200
        E = []; F = []; e = Dict(2=>[-999999999,0],3=>[0,99999999]); fval = [];
        ...
        while true
            while true
                ...
                Time_getCon = @CPUelapsed ϵ,ϵ′ = getConstraints(m,e,E)
                Times[1] = Times[1]+Time_getCon
                # print("new epsilons= ",ϵ,ϵ′,"\n")
                if [ϵ,ϵ′] ∉ F
                    Time_opt= @CPUelapsed s,objs,solvedIP = opt(lexi1,lexi2,lexi3,data,ϵ,ϵ′,solvedIP)
                    Times[2] = Times[2]+Time_opt
                    Time_domi = @CPUelapsed dominance = dominated(objs,fval)
            ...
            Time_updateCon = @CPUelapsed dummy = updateConstraints(objs,e)
...
epTime = @CPUelapsed epsol,epIPcount,Timerecord = Laumanns()
...
insertcols!(record1,8, Symbol("$colname")=>epTime)
epX=DataFrame(matE); epY=DataFrame(dfE);

CSV.write("/home/k2g00/k2g3475/multiobjective/solvers/ep+FP/ep_results/ep_fixedcost_"*"$ins"*"_record.csv",record1, append=true, writeheader=false)
#CSV.write(ARGS[1]*"ep_fix_record.csv", record1, writeheader=false)
#CSV.write(ARGS[1]*"ep_X.csv", epX, append=true, writeheader=false)
#CSV.write(ARGS[1]*"ep_inf_Y.csv",epY, writeheader=false, delim=' ' )
